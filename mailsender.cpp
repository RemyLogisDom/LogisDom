...
QString MailSender::mailData()
{
   ...
   // data.append("This is a multi-part message in MIME format.\r\n\n");
    ...
    //data.append(codec->fromUnicode(_body) + "\r\n");
    data.append(_body + "\r\n");
...
bool MailSender::send()
{
    ...
    qDebug() << QString("Port %1").arg(_port);
    ...
    //if( !sendCommand("AUTH CRAM-MD5", "334") ) {
        return false;
	}
    //QString Hash = _lastResponse.remove(0, 4);
    //QByteArray hash = QCryptographicHash::hash(Hash.toUtf8(),QCryptographicHash::Md5);
    //QString boundary = hash.toHex();
